...
export async function handleFindObjectsByCategory(req: FastifyRequest, res: FastifyReply) {
  const paramsValidation = z.object({
    category: z.union([z.nativeEnum(Category).array(), z.nativeEnum(Category)]).optional(),
  })
  ...
  for await (const obj of objects) {
    const command = new GetObjectCommand({ Bucket: process.env.R2_BUCKET, Key: obj.imageKey! })
